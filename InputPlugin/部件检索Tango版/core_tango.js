let tangoAPI = new TangoApi(serverUrl);
let db_table_dt = 'tb_input_plugin_dt';
let db_table_alt_components = 'tb_input_plugin_alt_components';
var wordObjs = [];
...
function prepareData(rebuild) {
    ...
    document.getElementById('button-settings').classList.remove("open");
    try {
...
function findMathViaAPI(maxNumberRetun, checkOnTheFlyOptions) {
    ...
    let summaryMsg = "「" + s + "」" + (resultWords.length > maxNumberRetun ? ("<span style='color:red'>超過 " + maxNumberRetun + " 字</span>") : ("總計 " + resultWords.length + " 字")) + " (" + Escalate + " 秒)";
    ...
    if (resultWords.length > 0) {
        ...
        for (var i = 0; i < resultWords.length; i++) {
            ...
            html += `<a class='out-char-block ${f} ${resultWords[i].BlockText}' oncontextmenu="ReplaceFind('${word}');return false;" onclick="sendWord('${word}')">${word}</a>`;
        }
